...
class FluxAttentionControl:
    ...
    def prepare_attention_mask(self, lin_masks: List[torch.Tensor], region_strengths: List[float], Nx: int, emb_size: int, emb_len: int):
        ...
        cross_mask = torch.zeros(total_len, total_len, dtype=torch.float32)
        q_scale = torch.ones(total_len, dtype=torch.float32)
        k_scale = torch.ones(total_len, dtype=torch.float32)
        ...
        for i in range(n_regs):
            ...
            m_with_tokens = torch.cat([torch.ones(emb_len, dtype=torch.float32), mask_i])
